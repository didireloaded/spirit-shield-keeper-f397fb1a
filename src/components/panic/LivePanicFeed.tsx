...
export function LivePanicFeed() {
  ...
  const fetchPanics = async () => {
    ...
    if (data && data.length > 0) {
      ...
      setPanics(data.map(p => ({ ...p, profile: profileMap.get(p.user_id) })));
    } else {
      setPanics([]);
    }
    setLoading(false);
        ...
        {panics.map((panic, index) => (
          ...
          >
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="font-medium text-sm">
                    {panic.current_location_name || panic.location_name || "Location pending..."}
                  </p>
                  {panic.is_moving && (
            ...
            <div className="flex gap-2">
              <button
                onClick={() => navigate(`/map?panic=${panic.id}`)}
